...
async fn main(_spawner: Spawner) {
...

    {
        ...
        // // let flash_id = flash.read_id_octa_dtr();
        // // info!("FLASH ID: {=[u8]:x}", flash_id);

        // let mut rd_buf = [0u8; 16];
        // flash.read_memory_octa_dtr(0, &mut rd_buf, false);
        // info!("READ BUF: {=[u8]:#X}", rd_buf);

        // info!("Erasing sector 0");
        // flash.erase_sector_octa_dtr(0).await;
        // flash.read_memory_octa_dtr(0, &mut rd_buf, false);
        // info!("READ BUF: {=[u8]:#X}", rd_buf);

        // info!("Writing to sector 0");
        // let mut wr_buf = [0u8; 16];
        // for i in 0..wr_buf.len() {
        //     wr_buf[i] = i as u8;
        // }
        // info!("WRITE BUF: {=[u8]:#X}", wr_buf);
        // flash.write_memory_octa_dtr(0, &wr_buf, true).await;
        // flash.read_memory_octa_dtr(0, &mut rd_buf, false);
        // info!("READ BUF: {=[u8]:#X}", rd_buf);
...
//     pub async fn new(hspi: Hspi<'static, I, Blocking>, mode: Mode) -> Self {
//         let mut memory = Self { hspi, mode };
...
//         self.write_enable().await;
//         self.write_cr2(Self::CR2_REG1_ADDR, Self::CR2_OCTA_DTR);
//     }
...
//     fn wait_write_finish(&mut self) {
//         match self.mode {
//             Mode::Spi => self.wait_write_finish_spi(),
//             Mode::OctaDtr => self.wait_write_finish_octa_dtr(),
//         }
//     }
...
//     pub fn read_memory_octa_dtr(&mut self, addr: u32, buffer: &mut [u8], use_dma: bool) {
//         let transaction = TransferConfig {
//             iwidth: HspiWidth::OCTO,
//             instruction: Some(Self::CMD_READ_OCTA_DTR as u32),
//             isize: AddressSize::_16Bit,
...
//     pub async fn erase_sector_octa_dtr(&mut self, addr: u32) {
//         info!("Erasing 4K sector at address: 0x{:X}", addr);
//         self.perform_erase_octa_dtr(addr, Self::CMD_SECTOR_ERASE_OCTA_DTR).await;
//     }

//     pub async fn erase_block_octa_dtr(&mut self, addr: u32) {
//         info!("Erasing 64K block at address: 0x{:X}", addr);
...
//     pub async fn write_memory_octa_dtr(&mut self, addr: u32, buffer: &[u8], use_dma: bool) {
//         let mut left = buffer.len();
//         let mut place = addr;
//         let mut chunk_start = 0;
